...
class AutoMessageScheduler:
    ...
    async def schedule_auto_messages(self, user_id: int):
        """Планирование автосообщений для Telegram"""
        if self.bot is None:
            auto_message_logger.error("Бот не установлен в AutoMessageScheduler")
            return

        messages = [
            (86400, "🔥 Полезно знать: 5 типичных ошибок студентов при написании кода."),  # 1 день
            (172800, "📢 Отзыв: 'Lab&Code помог мне защитить диплом по Python!'"),  # 2 дня
            (259200, "⏳ Осталось 3 места на сентябрьский курс Python. Успей записаться!")  # 3 дня
        ]
    ...
    async def schedule_vk_messages(self, user_id: int, vk_bot):
        """Планирование автосообщений для VK"""
        messages = [
            (86400, "🔥 Полезно знать: 5 типичных ошибок студентов при написании кода."),  # 1 день
            (172800, "📢 Отзыв: 'Lab&Code помог мне защитить диплом по Python!'"),  # 2 дня
            (259200, "⏳ Осталось 3 места на сентябрьский курс Python. Успей записаться!")  # 3 дня
        ...
        auto_message_logger.info(f"Запланировано {len(messages)} VK автосообщений для пользователя {user_id}")

    async def send_auto_message(self, user_id: int, message: str, delay: int):
        """Отправка запланированного сообщения в Telegram"""
        try:
    ...
    async def send_vk_auto_message(self, user_id: int, message: str, delay: int, vk_bot):
        """Отправка запланированного сообщения в VK"""
        try:
            await asyncio.sleep(delay)
            vk_bot.send_message(user_id, message)
            auto_message_logger.info(f"VK автосообщение отправлено пользователю {user_id}")
        except Exception as e:
            auto_message_logger.error(f"Ошибка отправки VK автосообщения пользователю {user_id}: {e}")
